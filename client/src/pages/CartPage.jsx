...
export default function CartPage() {
  ...
  return (
    <section className="min-h-screen px-8 py-20 bg-black text-white">
      ...
      {cartItems.length === 0 ? (
        <p className="text-center text-lg">🛒 Your cart is empty. <Link to="/" className="text-yellow-500 underline">Go shopping</Link>.</p>
      ) : (
        <div className="max-w-4xl mx-auto space-y-6">
          {cartItems.map((item) => (
            <div key={item.id} className="flex items-center justify-between bg-white/10 p-4 rounded-lg border border-yellow-700">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-lg font-semibold text-yellow-300">{item.name}</h4>
                  <p className="text-sm text-white/80">{item.content} | {item.volume} | ₹{item.rate}</p>
                </div>
              </div>
              <button
          ...
          <div className="text-right mt-6">
            <p className="text-xl font-bold text-yellow-300">
              Total: ₹{cartItems.reduce((sum, item) => sum + item.rate, 0)}
            </p>
